...
VIEW_W = GRID_W * TILE_SIZE
VIEW_H = GRID_H * TILE_SIZE
...
def pygame_loop():
    pygame.init()
    screen = pygame.display.set_mode((VIEW_W*2, VIEW_H))
    ...
    while editor.running:
                ...
                if mx < VIEW_W:
                    ...
                    if 0 <= gx < GRID_W and 0 <= gy < GRID_H:
                        ...
                        if prefab:
                            ...
                            print("Segmentos aplicados desde buffer")
                            redraw_map()
        ...
        # cielo
        pygame.draw.rect(screen, (80, 120, 200), (VIEW_W, 0, VIEW_W, VIEW_H//2))

        # piso
        pygame.draw.rect(screen, (40, 40, 40), (VIEW_W, VIEW_H//2, VIEW_W, VIEW_H//2))
        # -------- RENDER 3D --------
        cast_rays(screen.subsurface((VIEW_W,0,VIEW_W,VIEW_H)), textures)
...
def start_ui():
    ...
    tk.Label(right_frame, text="Segmentos en edición").pack()

    current_segments_listbox = tk.Listbox(right_frame)
    current_segments_listbox.pack(fill="both", expand=True)
    current_prefab_label = tk.Label(right_frame, text="Prefab: ninguno")
    current_prefab_label.pack()
    preview_canvas = tk.Canvas(right_frame, width=120, height=300, bg="black")
    ...
    library_listbox = tk.Listbox(right_frame)
    ...
    tk.Button(bottom_frame, text="Guardar Librería", command=save_segment_library).pack(fill="x")
    tk.Button(bottom_frame, text="Cargar Librería", command=load_segment_library).pack(fill="x")

    tk.Button(left_frame, text="Eliminar segmento", command=delete_segment).pack(fill="x")


    tk.Button(bottom_frame, text="Guardar mapa", command=save_map).pack(fill="x")
    tk.Button(bottom_frame, text="Cargar Mapa", command=load_map).pack(fill="x")
    tk.Button(bottom_frame, text="Exportar BIN", command=save_bin).pack(fill="x")
    tk.Button(bottom_frame, text="Exportar Mesh JSON", command=export_mesh_json).pack(fill="x")
    tk.Button(bottom_frame, text="Exportar Mesh BIN", command=export_mesh_bin).pack(fill="x")
